display_number = (print_number) =>{
    const like_log_cc=document.getElementById("like_log_cc");
...
(main = () =>{
    let random_number = generate_number();
    ...
    $("#like_log_log").on("click",function (e) {
        random_number == $("#like_log_in").val () 
            || alert("일련번호가 일치하지 않습니다.")
            || e.preventDefault()
            || $("#like_log_in").val ("").focus().css("background-color","rgba(247, 231, 125, 0.75)");
    })
})();