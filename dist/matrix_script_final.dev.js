...
function make_matrix(cell_id, x_line, y_line, box_id) {
  //matrix를 만드는 함수
  ...
  this.make_cell = function () {
    // 만들어진 배열 값을 셀로 만드는 배열
    var i = 0;
    var j = 0;
    ...
    while (i < this.x_line) {
      ...
      while (j < this.y_line) {
        //그 값을 만들어진 배열에 cell_id로 지정해서 다시 담아줌.
  ...
  this.print_cell = function (arg1, arg2) {
    // 화면에 출력하는 함수. 
  ...
  this.rename_id = function () {
    // 만들어진 행렬들에게 다시 id값을 부여해줌. 후에 행렬의 셀들이 서로 연산을 함. 
    this.in_array = [];
    var i = 0;
    var j = 0;
  ...
  this.cal_mix = function (matrix_1, matrix_2, cal_method) {
    ...
    if (matrix_1.length == matrix_2.length && matrix_1[0].length == matrix_2[0].length) {
      // matrix_1, matrix_2의 행과 열의 값이 같아야 함. 
      var i = 0;
      var j = 0;
  ...
  this.mix_multi = function (matrix_1, matrix_2) {
    //rename_id로 만들어지 행렬 2개의 각 셀들을 곱셈으로 연산함.
    if (matrix_1.length == matrix_2[0].length) {
      ...
      while (i < matrix_1.length) {
        // 여기에서 값이 0인 행렬을 미리 만들어둠. 안 만들시에 NaN 오류가 뜸.
        this.in_array.push([]);
  ...
  this.make_comma = function (arg) {
    //행렬의 각각 id 값을 받아서, 세자리 콤마형태로 만드는 함수. 
    var myNR = [];
    var myF = 1;
    var resultString = "";
...
(void 0).addEventListener('click', function (e) {
  var xX = parseInt(document.getElementById("xX").value); //Matrix1 X축

  var xY = parseInt(document.getElementById("xY").value); //Matrix1 Y축

  var xX1 = parseInt(document.getElementById("xX1").value); //Matrix2 의 X축

  var xY1 = parseInt(document.getElementById("xY1").value); //Matrix2 의 Y축

  var displayArea = document.getElementById("displayArea"); //Matrix1 의 출력하는 구역

  var displayArea1 = document.getElementById("displayArea1"); //Matrix2 의 출력하는 구역

  var get_matrix1 = new make_matrix("cell_one", xX, xY, displayArea);
  var get_matrix2 = new make_matrix("cell_two", xX1, xY1, displayArea1);
  var mix_calculate = new make_matrix("cell_one", xX1, xY1, displayArea2);
  ...
  switch (e.target.id) {
    ...
    case "plus_button":
      // 만들어진 2개의 행렬들에게(rename_id)로 다시 셀을 부여해주고, plus로 연산하게 함.
      mix_calculate.cal_mix(get_matrix1.rename_id(), get_matrix2.rename_id(), "plus");
      break;

    case "minus_button":
      // 만들어진 2개의 행렬들에게(rename_id)로 다시 셀을 부여해주고, minus로 연산하게 함.
      mix_calculate.cal_mix(get_matrix1.rename_id(), get_matrix2.rename_id(), "minus");
      break;

    case "multi_button":
      // mix_multi라는 메소드를 이용하여 rename_id로 셀을 재부여하고, 값인 0인 행렬에 곱셈이 된&콤마화된 값을 출력함. 
