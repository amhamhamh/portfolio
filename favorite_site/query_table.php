        ...
        if(!$conn){
            ...
            if(!mysqli_select_db($conn, "c12st20")){
                ...
                if(isset($_GET['action'])){       
                    ...
                    switch($_GET['action']){            
                        case 'insert':                               
                        $sql1 = "INSERT INTO favorite_site VALUES ('','$num1', '$num2', '$num3', '$num4', '$num5', '$num6')";
                        break;    
                        case 'delete':
                        $sql1 = "DELETE FROM favorite_site where name='".$num1."'";
                        break;    
                        case 'update':
                        $sql1 = "UPDATE favorite_site SET name='{$num1}', introduction='{$num2}',  date='{$num3}', address='{$num4}', division='{$num5}', etc='{$num6}' where name='{$num1}'";
                        break;
                        case 'search':
                        $sql1 = "SELECT * FROM favorite_site where name='".$num1."'";        
                    }    
                    ...
                    if(!$result){
                        ...
                        if(mysqli_num_rows($result) == 0){
                            ...
                            while($row = mysqli_fetch_assoc($result)){
                                echo "<tr><td>{$row['id']}</td><td>{$row['name']}</td><td>{$row['introduction']}</td><td>{$row['date']}</td><td>{$row['address']}</td><td>{$row['division']}</td><td>{$row['etc']}</td></tr>";
                            }
                            echo "</table>";
